...
class App extends Component {
  ...
  componentDidMount = () => {
    this.timeId = setInterval(this.decTime, 1000)
  }
  ...
  render() {
    const {currentTabId, randomImageUrl, score, time, scorecard} = this.state
    const filteredImagesList = imagesList.filter(eachImage =>
      eachImage.category.includes(currentTabId),
    )

    let ImagesOrScoreCard
    if (scorecard === true || time === 0) {
      ImagesOrScoreCard = (
      ...
      ImagesOrScoreCard = (
        <>
          <div className="random-img-container">
            <img src={randomImageUrl} className="random-img" alt="match" />
          </div>
          <ul className="ul-tabsList-container">
            {tabsList.map(eachTab => (
              <TabsComponents
                eachTab={eachTab}
                updatingTabId={this.updatingTabId}
                isActive={currentTabId === eachTab.tabId}
                key={eachTab.tabId}
              />
            ))}
          </ul>
          <ul className="ul-imagesList-container">
            {filteredImagesList.map(eachImage => (
              <ImageComponent
                eachImage={eachImage}
                key={eachImage.id}
                matchThumbnailWithRandomImage={
                  this.matchThumbnailWithRandomImage
                }
              />
            ))}
          </ul>
